...
struct TrickPileView: View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                ...
                .frame(
                    width: GameConstants.trickPileDiameter, height: GameConstants.trickPileDiameter)
            ...
            if let active = activeTrick {
                ForEach(Array(active.cards.enumerated()), id: \.offset) { _, play in
                    ...
                    CardView(
                        card: play.card, isPlayable: false, isDimmed: false,
                        width: GameConstants.trickPileCardWidth
                    )
        ...
        .animation(
            .timingCurve(0.2, 0.8, 0.2, 1, duration: GameConstants.trickGatherAnimationDuration),
            value: trick == nil
        )  // Toplanma için akıcı Curve
    ...
    func offsetFor(_ dir: TableDirection) -> CGSize {
        switch dir {
        case .south: return CGSize(width: 0, height: GameConstants.trickPileOffset)
        case .north: return CGSize(width: 0, height: -GameConstants.trickPileOffset)
        case .west: return CGSize(width: -GameConstants.trickPileOffset - 40, height: 0)
        case .east: return CGSize(width: GameConstants.trickPileOffset + 40, height: 0)
        }
    }
    ...
    func gatherOffset(_ dir: TableDirection) -> CGSize {
        switch dir {
        case .south: return CGSize(width: 0, height: GameConstants.trickGatherOffset)
        case .north: return CGSize(width: 0, height: -GameConstants.trickGatherOffset)
        case .west: return CGSize(width: -GameConstants.trickGatherOffset - 180, height: 0)
        case .east: return CGSize(width: GameConstants.trickGatherOffset + 180, height: 0)
        }
    }
}
